...
class ProjectFirestoreManager:
    def __init__(
        ...
        *,
        ...
        if firebase_config_json_path:
            ...
            # We still need a bucket name; don’t guess. Require env/secret for bucket in this mode.
            if not bucket_name:
        ...
        # Debounce flag to prevent duplicate writes in same rerun
    ...
    def _excel_blob_path(self, project_id: str, filename: str) -> str:
        # filename typically original upload name or 'grid.xlsx'
    ...
    def _upload_bytes(self, blob_path: str, data: bytes, content_type: str) -> str:
        ...
        # Making public may require bucket to allow public access; if it fails, we just skip.
        try:
            blob.make_public()
            return blob.public_url  # usually https://storage.googleapis.com/<bucket>/<path>
        except Exception:
            # Fallback: return a gs:// path to at least expose where it lives
...
def integrate_with_streamlit_app() -> Optional[ProjectFirestoreManager]:
    ...
    try:
        ...
        if os.path.exists("serviceAccount.json"):
            ...
            # bucket_name is required even with local JSON to avoid guessing wrong buckets
